"""
Enhanced LSTM Model Training Script
Used to train the Enhanced LSTM model with self-attention and residual connections.
"""
...
class EnhancedLSTMTrainer:
    ...
    def build_model(self):
        ...
        # Create model
        self.model = create_enhanced_lstm_model(
    ...
    def evaluate(self):
        ...
        # Move predictions and actuals to CPU
...
def main():
    ...
    # The comprehensive runner uses task_type, so we change it here for consistency
    ...
    # Convert task_type to the format expected by get_config ('short' or 'long')
    task_for_config = 'short' if args.task_type == 'short_term' else 'long'

    print(f"Running Enhanced LSTM model, Task: {args.task_type}, Model Size: {args.model_size}")

    config = get_config('enhanced_lstm', task_for_config, model_size=args.model_size)
    ...
    if args.results_dir:
        ...
        config['results_dir'] = f"results/enhanced_lstm_{args.task_type}{model_size_str}_{timestamp}"
